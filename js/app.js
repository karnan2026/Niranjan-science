...
function renderQuizTab(chapter, key, opts) {
  ...
  if (q.done) {
    wrap.appendChild(renderScorePanel(q, () => { state.quiz = { ...makeQuizState(items), key }; renderHub(); }));
    return wrap;
  }
  ...
  item.options.forEach((optText, idx) => {
    ...
    optBtn.addEventListener('click', () => {
      ...
      q.answers[q.i] = idx;
      if (idx === item.correctIndex) q.correctCount++;
      renderHub();
    });
...
function renderScorePanel(q, onRestart) {
